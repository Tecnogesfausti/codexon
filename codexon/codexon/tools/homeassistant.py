...
async def resolve_entity_reference(
    ...
    *,
    ...
    if not reference:
        return "", warnings
    memory = getattr(context, "memory", None)
    taught_resolver = getattr(memory, "resolve_taught_entity", None)
    if callable(taught_resolver):
        taught = taught_resolver(reference, domain=domain)
        if taught and taught != reference:
            warnings.append(f"entity_taught:{reference}->{taught}")
            reference = taught
...
async def ha_teach_entity_mapping(context: Any, args: dict[str, Any]) -> str:
    memory = getattr(context, "memory", None)
    if memory is None or not hasattr(memory, "teach_entity_mapping"):
        raise RuntimeError("La memoria semántica no admite enseñanzas de entidades")
    operation = str(args.get("operation") or "").strip().lower()
    notes = str(args.get("notes") or "").strip() or None

    if operation == "list":
        rows = memory.list_entity_teachings(
    ...
    if operation == "remove":
        teaching_type = str(args.get("teaching_type") or "alias").strip().lower()
        key_text = str(args.get("alias") or args.get("old_entity_id") or "").strip()
        if teaching_type not in {"alias", "replacement"} or not key_text:
            raise ValueError("Para quitar una enseñanza indica tipo y alias/entidad anterior")
        removed = memory.remove_entity_teaching(
    ...
    if operation == "alias":
        ...
        target_reference = str(
            args.get("target_entity_id") or args.get("target_query") or ""
        ).strip()
        if not alias or not target_reference:
            raise ValueError("Para enseñar un alias indica alias y entidad destino")
        target, warnings = await resolve_entity_reference(
            context,
            entity_id=target_reference if "." in target_reference else "",
            query="" if "." in target_reference else target_reference,
            domain=str(args.get("domain") or ""),
        )
        if not target or "." not in target:
            raise ValueError(f"No se pudo verificar la entidad destino: {target_reference}")
        memory.teach_entity_mapping(
            teaching_type="alias",
            key_text=alias,
    ...
    if operation == "replace":
        old_reference = str(
            args.get("old_entity_id") or args.get("old_query") or ""
        ).strip()
        new_reference = str(
            args.get("target_entity_id") or args.get("target_query") or ""
        ).strip()
        if not old_reference or not new_reference:
            raise ValueError("Para sustituir indica entidad anterior y entidad nueva")
        if "." in old_reference:
            old_entity = old_reference.lower()
        else:
            old_entity, _ = await resolve_entity_reference(
                context,
                query=old_reference,
                domain=str(args.get("domain") or ""),
            )
        new_entity, warnings = await resolve_entity_reference(
            context,
            entity_id=new_reference if "." in new_reference else "",
            query="" if "." in new_reference else new_reference,
            domain=str(args.get("domain") or ""),
        )
        if not old_entity or "." not in old_entity:
            raise ValueError(f"No se pudo resolver la entidad anterior: {old_reference}")
        if not new_entity or "." not in new_entity:
            raise ValueError(f"No se pudo verificar la entidad nueva: {new_reference}")
        memory.teach_entity_mapping(
            teaching_type="replacement",
            key_text=old_entity,
            target_entity_id=new_entity,
            source="conversation",
            notes=notes,
        )
        return compact_json(
    ...
    raise ValueError("operation debe ser alias, replace, remove o list")
...
async def ha_search_site_entities(context: Any, args: dict[str, Any]) -> str:
    """Resolve an entity collection from portable site-profile metadata."""
    profile = getattr(context, "site_profile", None)
    if profile is None or not hasattr(profile, "search_bindings"):
        return compact_json(
            {
                "count": 0,
                "entities": [],
                "read_only": True,
                "warning": "No hay perfil semántico local configurado.",
            }
        )
    query = str(args.get("query") or "").strip()
    role_prefix = str(args.get("role_prefix") or "").strip()
    kind = str(args.get("kind") or "").strip()
    limit = max(1, min(int(args.get("limit") or 100), 500))
    bindings = profile.search_bindings(
        query,
    ...
    states = await fetch_ha_states(context)
    state_by_id = {
        str(state.get("entity_id") or ""): state
        for state in states
        if state.get("entity_id")
    }
    entities: list[dict[str, Any]] = []
    entity_index: dict[str, int] = {}
    simultaneous_limits: list[int] = []
    for role, binding in bindings:
        label = str(binding.get("label") or role)
        binding_kind = str(binding.get("kind") or "")
        area = str(binding.get("area") or "")
        tags = [str(item) for item in binding.get("tags") or []]
        aliases = [str(item) for item in binding.get("aliases") or []]
        configured_limit = binding.get("max_simultaneous")
        if isinstance(configured_limit, int) and configured_limit > 0:
            simultaneous_limits.append(configured_limit)
        for entity_id in profile.entities(role):
            state = state_by_id.get(entity_id, {})
            attrs = state.get("attributes") or {}
            semantic_bindings = [
                (candidate_role, candidate_binding)
                for candidate_role, candidate_binding in profile.roles.items()
                if candidate_role != role
                and entity_id in profile.entities(candidate_role)
            ]
            specific_binding = next(
                (
                    (candidate_role, candidate_binding)
                    for candidate_role, candidate_binding in semantic_bindings
                    if str(candidate_binding.get("kind") or "") != "collection"
                ),
                None,
            )
            specific_role = specific_binding[0] if specific_binding else None
            specific_data = specific_binding[1] if specific_binding else {}
            if entity_id in entity_index:
                row = entities[entity_index[entity_id]]
            ...
            entities.append(
                {
                    ...
                    "friendly_name": attrs.get("friendly_name"),
                    "label": str(
                        specific_data.get("label")
                        or attrs.get("friendly_name")
                        or entity_id
                    ),
                    "roles": [
                        role,
                        *[candidate_role for candidate_role, _ in semantic_bindings],
                    ],
                    "kind": str(specific_data.get("kind") or binding_kind) or None,
                    "area": area or None,
                    "tags": tags,
                    "aliases": [
                        *aliases,
                        *[str(item) for item in specific_data.get("aliases") or []],
                    ],
                    "specific_role": specific_role,
                    "semantic_name_known": specific_binding is not None,
                    "assignment_note": (
                        None
                        if specific_binding is not None
                        else "Sin zona o función específica asignada en el perfil."
                    ),
                }
            )
        ...
        if len(entities) >= limit:
            break
    entities.sort(key=lambda row: natural_entity_sort_key(str(row["entity_id"])))
    return compact_json(
        {
            "count": len(entities),
            ...
            "entities": entities,
            "read_only": True,
            "simultaneous_limit_known": bool(simultaneous_limits),
            "max_simultaneous": min(simultaneous_limits) if simultaneous_limits else None,
            "constraint_note": (
                "El perfil no define cuántos pueden activarse simultáneamente; no lo infieras."
                if not simultaneous_limits
                else "Límite simultáneo definido expresamente en el perfil."
            ),
        },
        max_chars=30000,
    )
...
def natural_entity_sort_key(entity_id: str) -> tuple[Any, ...]:
    return tuple(
        int(part) if part.isdigit() else part
        for part in re.split(r"(\d+)", entity_id)
    )
...
def history_point_time(point: dict[str, Any]) -> dt.datetime | None:
    return parse_ha_datetime(str(point.get("last_updated") or point.get("last_changed") or ""))


def active_history_intervals(
    points: list[dict[str, Any]],
    *,
    ...
    for point in sorted(points, key=lambda item: history_point_time(item) or start_at):
        when = history_point_time(point)
        if when is None or when < start_at or when >= end_at:
            continue
        state = str(point.get("state") or "").strip().lower()
        if state == active_state and opened_at is None:
            opened_at = when
        elif state != active_state and opened_at is not None:
            intervals.append((opened_at, when))
            opened_at = None
    if opened_at is not None:
        intervals.append((opened_at, end_at))
    return intervals


def positive_numeric_delta(
    ...
    *,
    ...
    for when, value in points:
        if when <= start_at:
            ...
            break
    if baseline is None or not selected:
        return None, 0, 0
    total = 0.0
    resets = 0
    previous = baseline
    for value in selected:
        ...
        if change >= 0:
            ...
            resets += 1
        previous = value
    return total, len(selected), resets


async def ha_measure_numeric_during_state(context: Any, args: dict[str, Any]) -> str:
    activity_entity_id = str(args.get("activity_entity_id", "") or "").strip()
    measurement_entity_id = str(args.get("measurement_entity_id", "") or "").strip()
    if not activity_entity_id or not measurement_entity_id:
        raise ValueError("activity_entity_id y measurement_entity_id son obligatorios")
    ...
    active_state = str(args.get("active_state", "on") or "on").strip().lower()

    activity_entity_id, _activity_warnings = await resolve_entity_reference(
        context, entity_id=activity_entity_id
    )
    measurement_entity_id, _measurement_warnings = await resolve_entity_reference(
        context, entity_id=measurement_entity_id, domain="sensor"
    )
    if not activity_entity_id or not measurement_entity_id:
        raise ValueError("No se pudieron resolver las entidades solicitadas")

    httpx = httpx_module(context)
    async with httpx.AsyncClient(timeout=45) as http:
        measurement_state_response = await http.get(
            f"{ha_base_url(context)}/api/states/{measurement_entity_id}",
            headers=ha_headers(context),
        )
        measurement_state_response.raise_for_status()
        measurement_state = measurement_state_response.json()
        histories: dict[str, list[dict[str, Any]]] = {}
        for entity_id in (activity_entity_id, measurement_entity_id):
            response = await http.get(
                f"{ha_base_url(context)}/api/history/period/{start_time}",
                headers=ha_headers(context),
                params={
            ...
            response.raise_for_status()
            histories[entity_id] = flatten_history_points(response.json())

    intervals = active_history_intervals(
        histories[activity_entity_id],
        start_at=start_at,
        end_at=end_at,
        active_state=active_state,
    )
    numeric_points: list[tuple[dt.datetime, float]] = []
    ignored_non_numeric = 0
    for point in useful_history_points(histories[measurement_entity_id]):
        when = history_point_time(point)
        if when is None:
            continue
        try:
        ...
        if math.isfinite(value):
            numeric_points.append((when, value))
        else:
            ignored_non_numeric += 1
    ...
    for interval_start, interval_end in intervals:
        value, samples, resets = positive_numeric_delta(
            numeric_points,
        ...
        interval_results.append(
            {
                "start": interval_start.isoformat(timespec="seconds"),
                "end": interval_end.isoformat(timespec="seconds"),
                "seconds": rounded_history_value((interval_end - interval_start).total_seconds()),
                "value": rounded_history_value(value) if value is not None else None,
                "samples": samples,
                "resets_ignored": resets,
            }
        )

    attributes = measurement_state.get("attributes") if isinstance(measurement_state, dict) else {}
    return compact_json(
        {
            ...
            "start_time": start_time,
            "end_time": end_time,
            "calculation": "sum(positive measurement increments while activity entity is active)",
            ...
            "coverage_complete": bool(intervals) and intervals_with_data == len(intervals),
            "ignored_non_numeric": ignored_non_numeric,
            "resets_ignored": total_resets,
            "total": rounded_history_value(total) if intervals_with_data else None,
