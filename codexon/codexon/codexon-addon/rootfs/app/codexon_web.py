...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .task-table { min-width: 1040px; table-layout: fixed; }
  ...
  <main>
    ...
    <div class="grid">
      ...
      <section class="full" data-page="tareas">
        ...
        <div class="toolbar"><button class="secondary" onclick="createTask()">Crear tarea</button><button class="danger" onclick="deleteAllTasks()">Eliminar todas</button></div>
        <div class="table-scroll"><table class="task-table"><colgroup><col class="col-id"><col class="col-status"><col class="col-plan"><col class="col-result"><col class="col-actions"></colgroup><thead><tr><th>ID</th><th>Estado</th><th>Plan</th><th>Resultado</th><th>Acciones</th></tr></thead><tbody id="tasks"></tbody></table></div>
      </section>
...
async function loadAll() {
  const [status, observations, tasks, monitor, logs, tools, agents, codex, notes, backups] = await Promise.all([
    requestJSON('/api/status'), requestJSON('/api/observations'), requestJSON('/api/tasks'), requestJSON('/api/task-monitor'), requestJSON('/api/logs'), requestJSON('/api/tools'), requestJSON('/api/agents'), requestJSON('/api/codex/context'), requestJSON('/api/codex/notes'), requestJSON('/api/backups')
  ]);
