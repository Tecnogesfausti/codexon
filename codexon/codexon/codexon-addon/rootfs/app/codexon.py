...
def critical_operation_category(user_text: str) -> str | None:
    folded = normalize_cancellation_key(user_text)
    security_terms = (
        "alarma", "sirena", "seguridad", "cerradura", "cancela", "camara",
...
class CodexonAgent:
    ...
    def __init__(
        ...
        *,
        ...
        self.tool_names.extend(
            ["schedule_task", "reschedule_current_task", "list_scheduled_tasks", "edit_scheduled_task", "cancel_scheduled_task", "wait_seconds"]
        )
    ...
    def resolve_site_alias(self, text: str) -> tuple[str, str] | None:
        ...
        return profile.resolve_alias(text) if profile is not None else None

    def site_entity(self, role: str, default: str | None = None) -> str | None:
        profile = getattr(self, "site_profile", None)
        return profile.entity(role, default) if profile is not None else default

    def site_entities(self, role: str, default: tuple[str, ...] = ()) -> tuple[str, ...]:
        profile = getattr(self, "site_profile", None)
        return profile.entities(role, default) if profile is not None else default
...
- Para acciones futuras de Home Assistant, condiciones sobre sensores, secuencias, esperas o acciones internas usa preferentemente schedule_automation con un plan estructurado AUTOMATION_PLAN_V1. El plan sirve igual para sensor, binary_sensor, atributos, service/delay y builtin steps: say_time, terminal_message, wait_seconds, ha_send_mobile_alert. Usa schedule_task solo para recordatorios de texto libre o casos que no puedan expresarse como conditions + steps. No prometas recordar algo sin crear una tarea persistente. Esto incluye sinónimos y frases de futuro: agenda, programa, planifica, recuérdame, avísame, mándame, notifícame, dentro de, en N minutos/horas, al cabo de, pasados, tras, después, luego, más tarde, a las, para las, mañana, esta tarde/noche, cuando, en cuanto, si se cumple, cada, todos los, hasta que cancele y hasta nueva orden.
...
- Para condiciones futuras tipo "cuando X pase, haz Y", "en cuanto X se active, haz Y", "cuando la temperatura sea mayor de 25 enciende Y" o "si X supera Y, avisa", no te limites a leer el valor actual. Crea una tarea con schedule_task para la próxima comprobación. La instrucción debe evaluar la condición con herramientas; si no se cumple, usar reschedule_current_task para volver a comprobar más tarde; si se cumple, ejecutar la acción y terminar.
...
""".strip()
    ...
    async def _ask_unlocked(self, user_text: str, *, task: str) -> str:
        ...
        for _ in range(MAX_TOOL_ROUNDS):
                ...
                if schedule_intent and not scheduled_task_created and not schedule_retry_sent:
                    ...
                    working_messages.append({
                        ...
                        "content": (
                            ...
                            "La respuesta no es válida si no creas una tarea persistente. Para acciones o condiciones "
                            "de Home Assistant usa schedule_automation; para recordatorios usa schedule_task."
                        ),
                    })
                    continue
            ...
            for call in tool_calls:
                ...
                if real_name in {"__builtin__:schedule_task", "__builtin__:schedule_automation"}:
                    try:
    ...
    def try_cancel_tasks_by_cancellation_key(self, user_text: str) -> str | None:
        ...
        if not cancelled_rows:
            return None
        ids = ", ".join(f"#{int(row['id'])}" for row in cancelled_rows)
        return f"He cancelado las tareas asociadas a la clave '{key}': {ids}."
    ...
    async def execute_scheduled_instruction(self, task_id: int, run_id: int, title: str, instruction: str) -> str:
        ...
        try:
            async with self.ask_lock:
                return await self._run_task_unlocked(prompt, require_tool=requires_tool)
        finally:
            self.current_task_id = None
            self.current_task_run_id = None
    ...
    def parse_water_liters_payload(self, instruction: str) -> dict[str, Any] | None:
        ...
        if "riego" not in folded and "regar" not in folded:
            return None
    ...
    async def _run_task_unlocked(self, user_text: str, *, require_tool: bool = False) -> str:
        working_messages = [
            {"role": "system", "content": self._system_prompt(user_text)},
            {"role": "user", "content": user_text},
        ]
        ...
        for _ in range(MAX_TOOL_ROUNDS):
            ...
            tool_calls = getattr(msg, "tool_calls", None)
            if not tool_calls:
                if require_tool and action_calls_executed == 0:
                    raise RuntimeError(
